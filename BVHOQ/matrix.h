...
///
template <typename T, size_t W, size_t H> matrix_base<T,W,H>::matrix_base() { std::fill(&_m[0][0], &_m[0][0] + W*H, T()); }
template <typename T, size_t W, size_t H> matrix_base<T,W,H>::matrix_base(const matrix_base<T,W,H>& other) { std::copy(&other._m[0][0], &other._m[0][0] + W*H, &_m[0][0]); }
template <typename T, size_t W, size_t H> matrix_base<T,W,H>::matrix_base(const T* data) { std::copy(data, data + W*H, &_m[0][0]); }
