                          ...
                          )
{
    ...
    __local uint totalhist[4];
    __local uchar  localhist[4*64];
    ...
    __local uchar* mylocalhist = localhist + 4 * localid;

    // Load single int4 value
    int4 localvals = safe_load_int4_intmax(in_keys, globalid, numelems);
    ...
    for (int bit = 0; bit <= 2; bit += 2)
    {
        ...
        // Count histogram
        ++mylocalhist[((localvals.x >> bitshift) >> bit) & 0x3];
        ++mylocalhist[((localvals.y >> bitshift) >> bit) & 0x3];
        ++mylocalhist[((localvals.z >> bitshift) >> bit) & 0x3];
        ++mylocalhist[((localvals.w >> bitshift) >> bit) & 0x3];

        atomic_inc(&totalhist[((localvals.x >> bitshift) >> bit) & 0x3]);
        atomic_inc(&totalhist[((localvals.y >> bitshift) >> bit) & 0x3]);
        atomic_inc(&totalhist[((localvals.z >> bitshift) >> bit) & 0x3]);
        atomic_inc(&totalhist[((localvals.w >> bitshift) >> bit) & 0x3]);
        ...
        // Unpack the value
        mylocalhist[0] = (uchar)((packed_key)       & 0xFF);
        mylocalhist[1] = (uchar)((packed_key >> 8)  & 0xFF);
        mylocalhist[2] = (uchar)((packed_key >> 16) & 0xFF);
        mylocalhist[3] = (uchar)((packed_key >> 24) & 0xFF);

        // Pull the data to corresponding offsets to LDS
        uchar current = (localvals.x >> (bitshift + bit)) & 0x3;
        keys[totalhist[current] + mylocalhist[current]] = localvals.x;
        mylocalhist[current]++;

        current = (localvals.y >> (bitshift + bit)) & 0x3;
        keys[totalhist[current] + mylocalhist[current]] = localvals.y;
        mylocalhist[current]++;

        current = (localvals.z >> (bitshift + bit)) & 0x3;
        keys[totalhist[current] + mylocalhist[current]] = localvals.z;
        mylocalhist[current]++;

        current = (localvals.w >> (bitshift + bit)) & 0x3;
        keys[totalhist[current] + mylocalhist[current]] = localvals.w;
        mylocalhist[current]++;
        ...
        /*localvals.x = keys[localid*4];
        localvals.y = keys[localid*4 + 1];
        localvals.z = keys[localid*4 + 2];
        localvals.w = keys[localid*4 + 3];*/
    }
    ...
    int bin = (localvals.x >> bitshift) & 0xF;
    atomic_inc(&histogram[bin]);
    bin = (localvals.y >> bitshift) & 0xF;
    atomic_inc(&histogram[bin]);
    bin = (localvals.z >> bitshift) & 0xF;
    atomic_inc(&histogram[bin]);
    bin = (localvals.w >> bitshift) & 0xF;
    atomic_inc(&histogram[bin]);
    ...
    int v = (localvals.x >> bitshift) & 0xF;
    int offset = in_histograms[groupid + v * numgroups] + (localid*4) - histogram[v];
    //if (offset < numelems)
    {
        out_keys[offset] = localvals.x;
    }

    v = (localvals.y >> bitshift) & 0xF;
    offset = in_histograms[groupid + v * numgroups] + (localid*4 + 1) - histogram[v];
    //if (offset < numelems)
    {
        out_keys[offset] = localvals.y;
    }

    v = (localvals.z >> bitshift) & 0xF;
    offset = in_histograms[groupid + v * numgroups] + (localid*4 + 2) - histogram[v];
    //if (offset < numelems)
    {
        out_keys[offset] = localvals.z;
    }

    v = (localvals.w >> bitshift) & 0xF;
    offset = in_histograms[groupid + v * numgroups] + (localid*4 + 3) - histogram[v];
    //if (offset < numelems)
    {
